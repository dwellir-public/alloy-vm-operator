...
class AlloyCharm(ops.CharmBase):
    ...
    def _on_config_changed(self, event):
        ...
        if self._configure():
            self.unit.status = ops.ActiveStatus("Alloy config updated and valid")
        else:
    ...
    def _configure(self) -> bool:
        ...
        try:
            # restart guarantees the new config is picked up even if Alloy doesn’t support reload or reload is flaky.
            # reload - so the service doesn’t keep running with stale state after the restart settles.
            alloy.restart()
            alloy.reload()
        except subprocess.CalledProcessError as exc:
    ...
    def _seed_last_good_config_from_disk(self) -> bool:
        """Populate last_good_config from disk when unset (Typically as part of initial reconcile)."""
        on_disk = self._read_config_from_disk()
        if on_disk is None:
            return False
        self._stored.last_good_config = on_disk
        self._stored.config_drifted = False
        logger.debug("Seeded last_good_config from %s as part of initial reconcile", DEFAULT_CONFIG_PATH)
        return True

    def _has_config_drift(self) -> bool:
        """Return True when on-disk config differs from the last good config. Detects local manual config changes."""
        if not self._stored.last_good_config:
            return False
        on_disk = self._read_config_from_disk()
        if on_disk is None:
    ...
    def _desired_custom_args(self) -> str:
        return self.config.get("custom_args", "--server.http.listen-addr=0.0.0.0:6987")
